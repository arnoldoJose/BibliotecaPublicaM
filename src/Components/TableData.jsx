...
import { getMont,getDate,dayLast } from './Function/FunctionalData';
const TableData = ({item}) => {
  ...
  const getId = async (e) => {
    ...
    if (!auth.auth) {
      Swal.fire({ icon: 'error', title: 'Oops...', text: 'Para hacer un prestamos inicia sesion o registrate' });
    }else{
  ...
  const reservation = async (e) => {
    let id = e.target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.id;
    if (!auth.auth) {
      Swal.fire({ icon: 'error', title: 'Oops...', text: 'Para hacer una reservacion inicia sesion o registrate' });
    }else{
      let data = await clienteAxios.get(`get/book/${id}`);
      if (data.status === 200) Swal.fire("Reservacion procesada", 'You clicked the button!', 'success');
  ...
  const createLoan = async () => {
    ...
    Swal.fire(`${data.data.messageCorrect}`, 'You clicked the button!', 'success');
      ...
      <table className="table table-striped">
        ...
        <tbody>
          <tr id={item._id}>
            ...
            {
              (item.amount === 0) ?
                (<td className="alert alert-danger" >No disponible</td>)
                :
                (<td>disponible</td>)
            }
                   ...
                   <div className="modal-body">
                    <form id="form-loan">
                      <div className="mb-3">
                        ...
                        <input type="text" onChange={handelChange} name="name_user" placeholder="Nombre / Apellido" className="form-control" id="recipient-name" />
                      </div>
                      <div className="mb-3">
                        <label htmlFor="message-text" className="col-form-label">Numero de Telefono:</label>
                        <input type="text" onChange={handelChange}  name="mobile_user" placeholder="Telefono" className="form-control" id="recipient-mobile" />
                      </div>
                    </form>
                   </div>
